...
import joblib
model=joblib.load(r"C:\VS CODE\internship_3rd_semester_24105117001\PROJECTS\UsebBikePrediction\model.lb") #here linear regression model we selected
model2=joblib.load(r"C:\VS CODE\internship_3rd_semester_24105117001\PROJECTS\UsebBikePrediction\model2.lb") #here svr regression model we selected
...
@app.route('/history') #home route
def history():
    return render_template("history.html")
@app.route('/about') #home route
def about():
...
def predict():
    ...
    if request.method == "POST":
        ...
        pred=round(pred)
        pred2 = model2.predict(lst).item()
        pred2=round(pred2)
    return render_template("project.html", prediction=pred,prediction_2=pred2)
